    ...
    stages {
       ...
       stage('Dock push') {
        ...
        stage('Deploy') {
		  ...
		  sh 'if [ "$(docker ps --filter ancestor="keganferreira/linux_papertradingplatform")" ];then DID=(docker ps --filter ancestor="keganferreira/linux_papertradingplatform" | cut "-d " -f1 | tail -1) ; docker stop $DID; docker rm $DID; fi'    
