...
<body>
	<nav class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0">
		...
		<ul class="navbar-nav px-3">
			<li class="nav-item text-nowrap"><a class="nav-link" href="/main/">Sign
					out</a></li>
		</ul>
	</nav>
				...
				<div class="table-responsive">
					<table class="table table-striped table-sm">
						...
						<tbody>
							...
							<c:forEach items="${activeStocks}" var="entry">
   								 <tr>
								 <td>${entry.getSymbol()}</td>
								 <td>${entry.getName()}</td>
								 <td>${entry.getPrice()}</td>
								 <td>${entry.getCurrency()}</td>
								 <td>${entry.getStockExchange()}</td>
								 <td>${entry.getVolume()}</td>
							</tr> 
							</c:forEach>
							<tr>
								 <td><form action="processAddStock" method="post">
										<input type="text" name="symbol" placeholder = "name"/>
										<input type="submit" value="ADD STOCK" />
									</form>
									
									<c:if test="${not empty errorStockNotFound}">
										<p>${errorStockNotFound}</p>
									</c:if>
								</td>
								 <td></td>
								 <td></td>
