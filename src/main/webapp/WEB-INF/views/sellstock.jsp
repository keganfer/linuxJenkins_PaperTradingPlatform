						...
						<thead>
							<tr>
								...
								<h3>SELL STOCK</h3>
								 ...
								 <form action="processSellStock" method="post">
								 ...
								 <br>
								 <br>
								<c:if test="${not empty errorDontOwnStock}">
										<p>${errorDontOwnStock}</p>
									</c:if>
						
						<tbody>
						<br>
							<c:forEach items="${ownedStocks}" var="entry">
   								 <tr>
								 <td>${entry.key.getSymbol()}</td>
								 <td>${entry.value}</td>
								 <td>${entry.key.getPrice()}</td>
								 <td>${entry.key.getStockExchange()}</td>
								 <td>${entry.key.getCurrency()}</td>
							</tr>
